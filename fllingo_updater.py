"""FL Lingo standalone updater.

This script is compiled to FLLingoUpdater.exe and shipped alongside the main
...
Usage (invoked programmatically by FL Lingo):
    FLLingoUpdater.exe --mode zip|installer --wait-pid <PID>
        --install-root <dir> --archive-path <file> --exe-path <FL-Lingo.exe>
        [--extract-root <dir>]
"""
...
def _message_box(text: str, title: str = "FL Lingo Update", flags: int = 0x40) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.mode == "installer":
            ...
            if not started:
                _message_box(
                    "FL Lingo konnte das Update-Installationsprogramm nicht starten.\n"
        ...
        if not started:
            _message_box("FL Lingo wurde aktualisiert. Bitte starte das Programm manuell neu.")
            return 2
        return 0
    except Exception as exc:
        _message_box(f"FL Lingo Update fehlgeschlagen.\n\n{exc}", flags=0x10)
        return 1
    finally:
